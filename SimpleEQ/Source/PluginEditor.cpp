...
void LookAndFeel::drawRotarySlider(juce::Graphics& g,int x, int y, int width, int height,
    ...
    g.setColour(enabled ? Colour(255u, 54u, 1u) : Colours::grey);
...
void LookAndFeel::drawToggleButton(juce::Graphics& g, juce::ToggleButton& toggleBtn, bool shouldDrawBtnAsHighlighted, bool shouldDrawBtnAsDown) {
    using namespace juce;
    if (auto* pb = dynamic_cast<PowerButton*>(&toggleBtn)) {
        Path powerBtn;
    ...
    else if (auto* pb = dynamic_cast<AnalyzerButton*>(&toggleBtn)) {
        auto color = ! toggleBtn.getToggleState() ? Colours::dimgrey : Colours::lawngreen;
        g.setColour(color);

        auto bounds = toggleBtn.getBounds();
        g.drawRect(bounds);
...
SimpleEQAudioProcessorEditor::SimpleEQAudioProcessorEditor(SimpleEQAudioProcessor& p)
    ...
    lowCutBypassBtnAttachment(audioProcessor.apvts, "LowCut Bypassed", lowCutBypassBtn),
    peakFreqSliderAttachment(audioProcessor.apvts, "Peak Freq", peakFreqSlider),
    peakGainSliderAttachment(audioProcessor.apvts, "Peak Gain", peakGainSlider),
    peakQualitySliderAttachment(audioProcessor.apvts, "Peak Quality", peakQualitySlider),
    peakBypassBtnAttachment(audioProcessor.apvts, "Peak Bypassed", peakBypassBtn),
    highCutFreqSliderAttachment(audioProcessor.apvts, "HighCut Freq", highCutFreqSlider),
    highCutSlopeSliderAttachment(audioProcessor.apvts, "HighCut Slope", highCutSlopeSlider),
    highCutBypassBtnAttachment(audioProcessor.apvts, "HighCut Bypassed", highCutBypassBtn),
    analyzerEnableBtnAttachment(audioProcessor.apvts, "Analyzer Enabled", analyzerEnableBtn)
{
    ...
    analyzerEnableBtn.setLookAndFeel(&lnf);
    
    auto safePtr = juce::Component::SafePointer<SimpleEQAudioProcessorEditor>(this);
    lowCutBypassBtn.onClick = [safePtr]() {
        if (auto* comp = safePtr.getComponent()) {
            auto bypassed = comp->lowCutBypassBtn.getToggleState();
            comp->lowCutFreqSlider.setEnabled(!bypassed);
            comp->lowCutSlopeSlider.setEnabled(!bypassed);
        }
    };
    peakBypassBtn.onClick = [safePtr]() {
        if (auto* comp = safePtr.getComponent()) {
            auto bypassed = comp->peakBypassBtn.getToggleState();
            comp->peakFreqSlider.setEnabled(!bypassed);
            comp->peakGainSlider.setEnabled(!bypassed);
            comp->peakQualitySlider.setEnabled(!bypassed);
        }
    };
    highCutBypassBtn.onClick = [safePtr]() {
        if (auto* comp = safePtr.getComponent()) {
            auto bypassed = comp->highCutBypassBtn.getToggleState();
            comp->highCutFreqSlider.setEnabled(!bypassed);
            comp->highCutSlopeSlider.setEnabled(!bypassed);
        }
    };

    analyzerEnableBtn.onClick = [safePtr]() {
        if (auto* comp = safePtr.getComponent()) {
            auto enabled = comp->analyzerEnableBtn.getToggleState();
...
SimpleEQAudioProcessorEditor::~SimpleEQAudioProcessorEditor()
{
    ...
    analyzerEnableBtn.setLookAndFeel(nullptr);
}
...
void SimpleEQAudioProcessorEditor::resized()
{
    ...
    analyzerEnableBtn.setBounds(analyzerEnableArea);
    ...
    lowCutBypassBtn.setBounds(lowCutArea.removeFromTop(25));

    lowCutFreqSlider.setBounds(lowCutArea.removeFromTop(lowCutArea.getHeight() * 0.5));
    lowCutSlopeSlider.setBounds(lowCutArea);

    highCutBypassBtn.setBounds(highCutArea.removeFromTop(25));
    highCutFreqSlider.setBounds(highCutArea.removeFromTop(highCutArea.getHeight() * 0.5));
    highCutSlopeSlider.setBounds(highCutArea);

    peakBypassBtn.setBounds(bounds.removeFromTop(25));
