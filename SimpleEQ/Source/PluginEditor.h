...
struct PathProducer {
    PathProducer(SingleChannelSampleFifo< SimpleEQAudioProcessor::BlockType>& scsf) :
        channelFifo(&scsf) {
            channelFFTDataGenerator.changeOrder(FFTOrder::order8192);
            monoBuffer.setSize(1, channelFFTDataGenerator.getFFTSize());

    }
    void process(juce::Rectangle<float>, double sampleRate);
...
private:
    ...
    PowerButton lowCutBypassBtn, peakBypassBtn, highCutBypassBtn;
    AnalyzerButton analyzerEnableBtn;

    using BtnAttachements = APVTS::ButtonAttachment;
    BtnAttachements
        lowCutBypassBtnAttachment,
        peakBypassBtnAttachment,
        highCutBypassBtnAttachment,
